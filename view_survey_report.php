...
while($row=$answers->fetch_assoc()){
	if($row['type'] == 'radio_opt') $ans[$row['question_id']][$row['answer']][] = 1;
	if($row['type'] == 'check_opt'){
		foreach(explode(",", str_replace(array("[","]"), '', $row['answer'])) as $v){
		$ans[$row['question_id']][$v][] = 1;
		}
	}
	if($row['type'] == 'textfield_s') $ans[$row['question_id']][] = $row['answer'];
}
?>
<style>
	.tfield-area{
	...
	<div class="row">
		<div class="col-md-4">
			<div class="card card-outline card-primary">
				<div class="card-header">
					<h3 class="card-title"><b>Судалгааны мэдээлэл</b></h3>
				...
				<div class="card-body p-0 py-2">
					<div class="container-fluid">
						...
						<p>Бөглөсөн хүний тоо: <b><?=number_format($taken) ?></b></p>
		...
		<div class="col-md-8">
			<div class="card card-outline card-success">
				<div class="card-header">
					<h3 class="card-title"><b>Судалгааны тайлан</b></h3>
					...
					<div class="callout callout-info">
						...
						<input type="hidden" name="type[<?=$row['id'] ?>]" value="<?=$row['type'] ?>">	
							...
							<?php foreach(json_decode($row['frm_option']) as $k => $v): 
								...
								<li>
									...
									<span class=""><?=isset($ans[$row['id']][$k]) ? count($ans[$row['id']][$k]) : 0 ?>/<?=$taken ?></span>
						...
						<?php else: ?>
							<div class="d-block tfield-area w-100 bg-dark">
								...
								<blockquote class="text-dark"><?=$val ?></blockquote>
								<?php endforeach; ?>
								<?php endif; ?>
							</div>
						<?php endif; ?>
...
<script>
	...
	$('#print').click(function(){
		start_load()
		var nw = window.open("print_report.php?id=<?=$id ?>","_blank","width=800,height=600")
			nw.print()
			setTimeout(function(){
				nw.close()
				end_load()
