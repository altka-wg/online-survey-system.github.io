...
Class Action {
	...
	function save_survey(){
		...
		if(empty($id)) $save = $this->db->query("INSERT INTO survey_set set $data");
		else $save = $this->db->query("UPDATE survey_set set $data where id = $id");
		if($save) return 1;
	}
	...
	function save_question(){
		extract($_POST);
			...
			}else $data .= ", frm_option='' ";
		if(empty($id)) $save = $this->db->query("INSERT INTO questions set $data");
		else $save = $this->db->query("UPDATE questions set $data where id = $id");
		if($save) return 1;
	}
	...
	function save_answer(){
		extract($_POST);
			foreach($qid as $k => $v){
				...
				if($type[$k] == 'check_opt') $data .= ", answer='[".implode("],[",$answer[$k])."]' ";
				else $data .= ", answer='$answer[$k]' ";
				$save[] = $this->db->query("INSERT INTO answers set $data");
			}
		$save = $this->db->query("UPDATE users set total_point=total_point + $point where id = $uid");
		if(isset($save)) return 1;
	}
